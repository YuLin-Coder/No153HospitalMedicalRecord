	...
	<body>
			<form action="addYishengxinxi.do" name="form1" method="post">
				      <table width="100%" border="1" align="center" cellpadding="3" cellspacing="1"  style="border-collapse:collapse" bgcolor="#F2FDFF">
						<tr bgcolor="#E7E7E7">
							<td height="30" colspan="2" background="images/table_header.gif">添加医生信息</td>
						</tr>
...
<script language=javascript >  
function hsgcheck() {
		var yishenggonghao = $("#yishenggonghao").val();
		if(yishenggonghao==""||(yishenggonghao.length<3||yishenggonghao.length>12)){
			 $("#clabelyishenggonghao").html("<font color=red>医生工号不能为空且长度在3～12位之间！</font>");
			 $("input[id=yishenggonghao]").focus();
			 $("#querenzhuce").attr("disabled","disabled");
			 return false;
		} 
		else
		{
			$("#clabelyishenggonghao").html("");
			$.ajax({
				url : "quchongYishengxinxi.do",
				type : "post",
				data : "yishenggonghao=" + yishenggonghao,
				dataType : "json",
				success:function(result){
				if(result.info=="ng"){
					$("#clabelyishenggonghao").html("<font color=red>医生工号已存在，请更换！</font>");
					$("input[id=yishenggonghao]").focus();
					$("#querenzhuce").attr("disabled","disabled");
					return false;
				}
				...
				error:function(){
					$("#clabelyishenggonghao").html("系统异常，请检查错误！");
					$("input[id=yishenggonghao]").focus();
					$("#querenzhuce").attr("disabled","disabled");
					return false;
				}
